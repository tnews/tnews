...
class LocalFavoriteRepository extends FavoriteRepository {
  ...
  Future<bool> add(XNews news) {
    return getAllFavoriteNews()
        ...
        .then((List<XNews> news) =>
            news.map((XNews item) => json.encode(item.toJson())).toList())
        .then((List<String> strNews) =>
            shared.setStringList(_StorageKeys.favorite_news, strNews));
  }
  ...
  Future<List<XNews>> getAllFavoriteNews() async {
    final List<String> jsonNews =
        shared.getStringList(_StorageKeys.favorite_news) ?? <String>[];
  ...
  Future<bool> isFavorite(String id) {
    return getAllFavoriteNews()
        .then((_) =>
            _.asMap().map((_, XNews item) => MapEntry<String, XNews>(item.id, item)))
